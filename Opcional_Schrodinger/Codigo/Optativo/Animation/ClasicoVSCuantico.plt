# 1. Analizamos el archivo
stats archivo_entrada using 1:2 nooutput

# 2. Configuración estética
set terminal pngcairo size 800,600 background "#ffffff" enhanced font "Sans,12"
set output video_out

set xlabel x_label_t
set ylabel y_label_t
set grid

# 3. Rangos

max_y = STATS_max_y
min_y = STATS_min_y

max_x = STATS_max_x
min_x = STATS_min_x


stats archivo_entrada_2 using 1:2 nooutput

max_y_2 = STATS_max_y
min_y_2 = STATS_min_y

max_x_2 = STATS_max_x
min_x_2 = STATS_min_x

if (max_y_2 > max_y) { max_y = max_y_2 }
if (min_y_2 < min_y) { min_y = min_y_2 }
if (max_x_2 > max_x) { max_x = max_x_2 }
if (min_x_2 < min_x) { min_x = min_x_2 }

rango_y = max_y - min_y


if ((rango_y*1000) < 1.0 || (rango_y == 0.0)) {
    margen_y = abs(STATS_max_y) * 0.5
    if (margen_y == 0) { margen_y = 0.1 }
} else {
    margen_y = rango_y * 0.05
}

set xrange [min_x : max_x]
set yrange [min_y - margen_y : max_y + margen_y]

# 4. Gráfica estática completa
plot archivo_entrada using 1:2 with points linecolor rgb "red" title "Clásico", archivo_entrada_2 using 1:2 with points linecolor rgb "blue" title "Cuántico"

unset output