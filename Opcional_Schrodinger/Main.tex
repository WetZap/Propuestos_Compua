...
\centering
    {\HRule{2 pt}} \\
    \vspace{0.5cm}
    {\Huge Simulación de la ecuación de Schrödinger dependiente del tiempo para el oscilador armónico cuántico.}  \\
...
Además, compararemos el comportamiento cuántico del sistema con el comportamiento clásico del oscilador armónico, mostrando así las diferencias entre ambos comportamientos y cómo la mecánica cuántica se diferencia de la mecánica clásica en este sistema. Así como el límite de la mecánica cuántica con la mecánica clásica, ilustrando cómo el comportamiento cuántico se aproxima al comportamiento clásico en ciertas condiciones.
...
El código que se empleará en este proyecto se encuentra disponible en el siguiente repositorio de GitHub: \url{https://github.com/WetZap/Propuestos_Computacional/tree/main/Opcional_Schrodinger}
...
Para evitar trabajar con las constantes que aparecen reescalaremos las coordenadas buscando eliminarlas, para ello $t\rightarrow \hbar\ t$ y $x\rightarrow x\ \dfrac{\hbar}{\sqrt{2m}}$, logrando así convertir $m = 0.5$ y $\hbar = 1$, a partir de aquí trabajaremos con estas coordenadas reescaladas.
...
Si resolvemos la ecuación de Schrödinger mediante el método de separación de variables\footnote{La resolución se encuentra en el apéndice.} llegamos a la expresión de la solución:
...
Para la parte espacial de la función de onda, tenemos una derivada segunda respecto de $x$, por lo que emplearemos el método de diferencias centrales\footnote{Explicación en el apéndice.} para esta parte, obteniendo así la siguiente expresión:
\begin{equation}
...
Donde $\psi_n(x)$ es la función de onda en el tiempo $t_n$ y $\psi_{n+1}(x)$ es la función de onda en el tiempo $t_{n+1}$, con $t_{n+1} = t_n + \Delta t$; el operador $\dfrac{(1 - i\ \dfrac{\Delta t}{2}\ \hat{H})}{(1 + i\ \dfrac{\Delta t}{2}\ \hat{H})}$ es el operador de evolución temporal que es unitario si $\hat{H}$ es hermítico, lo que garantiza la conservación de la norma de la función de onda a lo largo del tiempo.
...
Operando sobre la expresión anterior y separando los términos de $q^n_j$ y los demás, obtenemos la siguiente ecuación, para reducir la cantidad de constantes definimos $\omega = \dfrac{\Delta t}{(\Delta x)^2}$:
\begin{equation}
    -\dfrac{i\ }{2\ }\omega\ q_{j+1}^{n} + (1 + i\ \dfrac{\Delta t}{2}\ V_j + i\ \omega) q_{j}^{n} - \dfrac{i\ }{2 }\omega\  q_{j-1}^{n} = 2\psi^n_j
\end{equation}

Sacando como factor común el término $i\ \omega/2$ llegamos a la siguiente expresión:
\begin{equation}
...
En nuestro caso, los valores $k_1$ y $k_3$ son constantes e igual a $1$, y siguiendo el procedimiento del apéndice llegamos a la expresión de $q^n_{j+1} = \alpha_j q^n_j + \beta_j$, con $\alpha_j$ y $\beta_j$ definidos por las siguientes fórmulas de recurrencia (para un $n$ fijo):
\begin{equation}
...
\subsection{Expresión de V(x). Oscilador Cuántico}
...
\begin{equation}\label{eq:Vx}
    V(x) = \dfrac{\omega_{osc}^2}{4} (x-x_0)^2
\end{equation}

Los parámetros que se han utilizado para el estudio del oscilador armónico cuántico son los siguientes:
\begin{itemize}
    \item $\omega_{osc} \in \{200, 800\ ,1500\}$: Frecuencia del oscilador armónico (se considera $200$ salvo indicación contraria).
    \item $x_0 = 0.5$: Posición de equilibrio del oscilador armónico.
    \item $x_0^{gauss} = 0.3$: Desplazamiento de la función gaussiana.
    \item $\sigma \in \{1/16, 1/10\}$: Ancho de la función gaussiana.
    ...
    \item $\Delta x =\ L/S\ =\ 0.001$: Paso espacial para la discretización del espacio.
\end{itemize}
...
\section{Resultados}
En esta sección se mostrarán los resultados obtenidos, se divide en tres sub-secciones, en las dos primeras cambiaremos las funciones de onda de entrada y observaremos su comportamiento mientras que en la última sub-sección compararemos el comportamiento clásico con el comportamiento cuántico.
...
Anteriormente se ha mencionado que con el método de Crank-Nicolson se garantiza la conservación de la norma de la función de onda a lo largo del tiempo; para comprobarlo se ha calculado la norma en cada paso de tiempo, comenzando por normalizar la función de onda en el instante inicial \ref{fig:codigo_norma}

\begin{figure}[H]
    \centering{
    \includegraphics[width=0.8\linewidth]{Image/Code/Norma.png}}
    \caption{Código para la normalización de la función de onda en el instante inicial.}
...
\begin{figure}[H]
    ...
    \href{https://wetzap.github.io/Propuestos_Computacional/Opcional_Schrodinger/Video/Auto/n_4/simulacion_Phi_Prob.mp4}{
...
En esta sección se ha calculado la parte real e imaginaria de la función de onda en cada paso de tiempo, para ello se usaron las funciones \texttt{real()} y \texttt{aimag()} de Fortran. En la siguiente gráfica se muestra la evolución de la parte real e imaginaria de la función de onda a lo largo del tiempo:


\begin{figure}[H]
    \centering

    \href{https://wetzap.github.io/Propuestos_Computacional/Opcional_Schrodinger/Video/Auto/n_4/simulacion_Juntos.mp4}{
...
Para el cálculo numérico del valor medio de $x$ y $p$, se han utilizado los equivalentes discretizados de las expresiones teóricas, a continuación se muestra la aplicación en nuestro programa:
...
Para el cá lculo numérico del valor medio de $H$ y el producto de las incertidumbres $\Delta x \Delta p$, se han utilizado los equivalentes discretizados de las expresiones teóricas, a continuación se muestra la aplicación en nuestro programa:
...
Como se puede observar en la gráfica \ref{fig:auto_valor_medio_H}, el valor medio de $H$ se mantiene constante en $\omega (n + \dfrac{1}{2})$ a lo largo del tiempo, mientras que el producto de las incertidumbres $\Delta x \Delta p$ se mantiene constante en $(n + \dfrac{1}{2})$. Aunque en el resultado de la incertidumbre se pueda observar que el valor numérico es ligeramente superior al valor teórico, esto se debe a que el valor numérico se ha calculado a través de la discretización de las expresiones teóricas, lo que introduce un error numérico en el cálculo, aunque este error es pequeño, es suficiente para que el valor numérico sea superior al valor teórico.
...
\begin{figure}[H]
    ...
    \href{https://wetzap.github.io/Propuestos_Computacional/Opcional_Schrodinger/Video/Func/simulacion_Phi_Prob.mp4}{
...
La gráfica que muestra la evolución de la probabilidad a lo largo del tiempo para nuestra funcón gaussiana, muestra un comportamiento oscilatorio, comportamiento que no presentaban las autofunciones del oscilador armónico cuántico, esto se debe a que la función gaussiana no es un estado estacionario del sistema, lo que significa que su forma cambia a lo largo del tiempo, lo que se refleja en la evolución de la probabilidad, aunque esta evolución es oscilatoria, la norma se mantiene constante a lo largo del tiempo.

Además se ha trazado la trayectoria que sigue el máximo de la densidad de probabilidad a lo largo del tiempo, evidenciando el comportamiento oscilatorio propio de un estado no estacionario.
...
\begin{figure}[H]
    \centering
    \href{https://wetzap.github.io/Propuestos_Computacional/Opcional_Schrodinger/Video/Func/simulacion_Juntos.mp4}{
...
Como se puede observar en la gráfica \ref{fig:gaus_medio_x_p}, el valor medio de $x$ y $p$ muestran un comportamiento oscilatorio a lo largo del tiempo. Este comportamiento es consecuencia directa de que la función gaussiana no es un estado estacionario del sistema: su forma cambia en cada instante, lo que se refleja en la dinámica de los valores esperados de posición y momento.
\subsubsection{Principio de incertidumbre y valor medio de $H$}
...
\begin{figure}[H]
    ...
    \href{https://wetzap.github.io/Propuestos_Computacional/Opcional_Schrodinger/Video/Auto/n_20/simulacion_Phi_Prob.mp4}{
    \includegraphics[width=0.8\linewidth]{Video/Auto/n_20/simulacion_Phi_Prob_captura.png}
    }
    \caption{Evolución de la probabilidad a lo largo del tiempo.}
    \label{fig:auto_time_n20}
\end{figure}
En este caso el valor de $\omega$ se ha cambiado a $800$ para que la evolución de la probabilidad sea más nítida, ya que con un valor de $\omega$ bajo la distribución oscila enormemente.
...
\begin{figure}[H]
    \centering
    \href{https://wetzap.github.io/Propuestos_Computacional/Opcional_Schrodinger/Video/Auto/n_80/simulacion_Phi_Prob.mp4}{
...
El error relativo es del orden de $10^{-14}$, lo que confirma que la energía se conserva con gran precisión durante la evolución. A pesar de haber igualado la energía inicial clásica a la cuántica, las trayectorias difieren entre sí, evidenciando que la concordancia energética no implica equivalencia dinámica entre ambas descripciones.



\section{Conclusiones}

En este trabajo se ha estudiado la evolución temporal de la función de onda para el oscilador armónico cuántico utilizando el método de Crank-Nicolson. Se han comparado los resultados para las autofunciones del sistema con los de una función gaussiana, evidenciando el contraste entre estados estacionarios y no estacionarios. Además, se ha analizado la correspondencia con el comportamiento clásico del oscilador armónico, comprobando cómo se recupera el límite clásico al aumentar el valor de $n$. Finalmente, se ha explicado la dinámica de los valores medios de $x$ y $p$ para la función gaussiana, ilustrando las diferencias respecto al oscilador clásico debidas a la naturaleza probabilística del sistema cuántico.
...
Esta ecuación se resolverá en la sección de resultados para el caso de las autofunciones \ref{subsec:Autofunciones}


\subsection{Aplicación del método de C-N para la ecuación de Schrödinger (solo discretización temporal).}

Sea la ecuación de Schrödinger unidimensional con ($\hbar = 1$ y $m = 0.5$):
\begin{equation}
    i\ \dfrac{\partial \Psi(x,t)}{\partial t} = [-\dfrac{\partial^2}{\partial x ^2} + V(x)]\Psi(x,t) = \hat{H} \Psi(x,t)
\end{equation}

La solución formal de esta ecuación incluye el operador de evolución temporal $e^{-i\ (t-t_0)\  H}$, el cual consume una gran cantidad de recursos computacionales, por ello empleamos el método de Crank-Nicolson en una ecuación del tipo $\partial_t \Psi = -i \hat{H}\Psi$. Discretizando el tiempo, obtenemos las siguientes expresiones, dependiendo de si el método es explícito o implícito:

Euler Explícito:
\begin{equation}
    \dfrac{\Psi_{n+1}(x) - \Psi_n(x)}{\Delta t} = -i \hat{H} \Psi_n(x) \Rightarrow \Psi_{n+1}(x) = \Psi_n(x) - i\ \Delta t\ \hat{H} \Psi_n(x)
\end{equation}
...
El método de Crank-Nicolson se basa en la media entre ambos métodos, obteniendo así la siguiente expresión:
\begin{equation}
...
Este método es de orden 2, lo que significa que el error de truncamiento es proporcional a $(\Delta t)^2$. Además, es un método incondicionalmente estable, lo que significa que no hay restricciones en el tamaño del paso de tiempo $\Delta t$  garantizando así la estabilidad numérica.
